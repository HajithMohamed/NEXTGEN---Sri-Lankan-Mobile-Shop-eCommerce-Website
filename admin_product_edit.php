...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($brand) || empty($category_id) || empty($price)) {
        ...
        if (empty($message)) {
            // Update product
            $stmt = $conn->prepare("UPDATE products SET name = ?, brand = ?, category_id = ?, description = ?, price = ?, image = ?, stock = ? WHERE id = ?");
            if ($stmt->execute([$name, $brand, $category_id, $description, $price, $image, $stock, $productId])) {
